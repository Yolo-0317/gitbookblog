...
// 增加cache后只计算一次
function curry(fn) {
  const length = fn.length; // 该函数期望传入的参数数量，即形参的个数，fn.length会返回形参个数
  const curryArgs = [...arguments].slice(1) || [];
  curry.cache = curry.cache || {};
  return function () {
    const newArgs = curryArgs.concat(Array.prototype.slice.call(arguments)); // 强制转化arguments为数组格式
